// Procedural dark-fantasy music using the Web Audio API.
...
class MusicManager {
    ...
    start() {
        ...
        this._master.gain.linearRampToValueAtTime(0.55, this.ctx.currentTime + 2.5);
        this._master.connect(this.ctx.destination);

        // Browser autoplay policy: AudioContext starts suspended.
        // Resume it on the very first user interaction.
        ...
        this._delay.delayTime.setValueAtTime(0.28, 0);
        this._feedback = this.ctx.createGain();
        this._feedback.gain.setValueAtTime(0.22, 0);
        this._wet      = this.ctx.createGain();
        this._wet.gain.setValueAtTime(0.22, 0);
    ...
    _scheduleLoop(t0) {
        ...
        // ── PAD (detuned sawtooth pairs — very soft, long sustain) ───────
        // Chord progression: Am  F  C  G  Am  F  E  Am (8 bars)
        const padChords = [
            [A2, C3, E3],   // Am
            [F2, A3, C3],   // F
            [C2, E3, G3],   // C
        ...
        // ── BASS LINE (triangle + octave sine) ───────────────────────────
        const bassNotes = [
            // Bar 1: Am
        ...
        bassNotes.forEach(([freq, tOff, dur, vol]) => {
            ...
            this._osc(freq*2, t0+tOff, dur*0.65, 'sine',     vol * 0.25);
        });

        // ── MELODY (square + sine blend for a medieval flute/horn feel) ──
        const mel = [
            // Bar 1: Am motif
            [E4, 0,              beat*0.5,  0.13],
            ...
            // Bar 5: Am — second half, more ornate
            ...
            // Bar 7: E — dramatic tension, high run
            ...
            [A3, bar*7+beat*2,   beat*2,    0.16],  // long final note
        ];
        ...
        // ── COUNTER-MELODY (soft triangle, bars 5-8 only) ────────────────
        const ctr = [
        ...
        for (let bi = 0; bi < 8; bi++) {
            ...
            // Open hi-hat accent on beat 3.5 in odd bars
            if (bi % 2 === 1) {
